...
export const Table = ({
  ...
  // Pagination (supports internal or server-side)
  const isServerSide = serverSideTotal !== undefined;
  const totalItems = isServerSide ? serverSideTotal : data.length;
  const activePage = isServerSide ? serverSidePage : currentPage;

  const totalPages = Math.max(1, Math.ceil(totalItems / rowsPerPage));
  const indexOfFirstItem = (activePage - 1) * rowsPerPage;
  const indexOfLastItem = isServerSide 
    ? indexOfFirstItem + data.length
    : Math.min(activePage * rowsPerPage, totalItems);
    
  const currentItems = isServerSide ? data : data.slice(indexOfFirstItem, indexOfLastItem);
  ...
  }, [data.length, totalPages, currentPage, isServerSide]);
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (isServerSide) {
                    if (onServerRowsChange) onServerRowsChange(newLimit);
