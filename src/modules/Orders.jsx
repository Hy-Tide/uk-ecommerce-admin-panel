...
export const Orders = ({
  ...
  React.useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (res && res.success && res.data?.orders && Array.isArray(res.data.orders) && isMounted) {
          const mapped = res.data.orders.map(c => {
            const ship = c.shippingAddress || {};
            const custName = ship.firstName || ship.lastName
              ? `${ship.firstName || ''} ${ship.lastName || ''}`.trim()
              : (c.user?.email || 'Customer');
            const addrStr = [ship.houseNumber, ship.street, ship.city, ship.county, ship.postcode].filter(Boolean).join(', ') || 'N/A';
            const payStatus = (c.paymentStatus === 'completed' || c.paymentStatus === 'paid')
              ? 'Paid'
              : (c.paymentStatus === 'refunded' ? 'Refunded' : 'Unpaid');

            return {
              id: c.orderNumber || c._id,
              _id: c._id,
              orderNumber: c.orderNumber,
              customerName: custName,
              customerEmail: ship.email || c.user?.email || 'N/A',
              phone: ship.phone || 'N/A',
              date: c.createdAt ? c.createdAt.split('T')[0] : new Date().toISOString().split('T')[0],
              deliveryStatus: c.orderStatus || 'Pending',
              paymentStatus: payStatus,
              total: c.totalAmount ?? c.subTotal ?? 0,
              subtotal: c.subTotal ?? c.totalAmount ?? 0,
              discount: c.discountAmount || 0,
              deliveryFee: c.shippingFee || 0,
              tax: 0,
              paymentMethod: c.paymentMethod ? c.paymentMethod.toUpperCase() : 'COD',
              address: addrStr,
              ...
              timeline: [
                { status: 'Order Placed', time: c.createdAt || new Date().toISOString(), desc: 'Order received from customer.' },
                { status: c.orderStatus || 'Pending', time: c.updatedAt || c.createdAt || new Date().toISOString(), desc: `Current status: ${c.orderStatus || 'Pending'}` }
              ]
            };
          });
          setOrders(mapped);
  ...
  const filtered = (orders || []).filter(o => {
    const orderIdStr = String(o?.id || o?.orderNumber || o?._id || '');
    const custNameStr = typeof o?.customerName === 'string' ? o.customerName : (typeof o?.customerName === 'object' ? (o.customerName?.name || '') : '');
    const matchSearch   = orderIdStr.toLowerCase().includes(search.toLowerCase()) || custNameStr.toLowerCase().includes(search.toLowerCase());
  ...
  const columns = [
    {
      ...
      render: row => <span style={{ fontFamily: 'monospace', fontWeight: '800', fontSize: '13px' }}>#{row?.id || row?.orderNumber || row?._id}</span>
    },
    {
      key: 'date', label: 'Date',
      render: row => {
        const dVal = row?.date || row?.createdAt;
        const dStr = dVal ? new Date(dVal).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A';
        return (
    ...
    },
    {
      ...
      render: row => {
        const cName = typeof row?.customerName === 'string' ? row.customerName : (typeof row?.customerName === 'object' ? (row.customerName?.name || 'Customer') : 'Customer');
        const initials = cName.split(' ').map(n => n[0]).filter(Boolean).join('').slice(0, 2) || 'C';
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontWeight: '600', fontSize: '13px' }}>{cName}</span>
          </div>
        );
      }
    },
    {
      ...
      render: row => <span style={{ fontWeight: '700', color: 'var(--primary)' }}>£{(Number(row?.total) || 0).toFixed(2)}</span>
    },
    {
      key: 'paymentStatus', label: 'Payment',
      render: row => <StatusPill label={row?.paymentStatus || 'Unpaid'} config={PAYMENT_CONFIG[row?.paymentStatus] || PAYMENT_CONFIG['Unpaid']} />
    },
    {
      key: 'deliveryStatus', label: 'Delivery',
      render: row => <StatusPill label={row?.deliveryStatus || 'Pending'} config={DELIVERY_CONFIG[row?.deliveryStatus] || DELIVERY_CONFIG['Pending']} />
    },
    {
      key: 'actions', label: '',
      render: row => (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%' }}>
          {[1, 2, 3, 4, 5, 6, 7].map(i => (
            <div key={i} className="shimmer" style={{ width: '100%', height: '52px', borderRadius: '12px' }} />
          ))}
        </div>
      ) : viewMode === 'list' ? (
        <ListView
        ...
        <GridView
          ...
          renderActions={item => (
            <>
              <span style={{ fontWeight: '800', color: 'var(--primary)' }}>£{(Number(item?.total) || 0).toFixed(2)}</span>
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                }}>{item?.paymentStatus || 'Unpaid'}</span>
