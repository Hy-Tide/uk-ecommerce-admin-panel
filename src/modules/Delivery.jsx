...
export const Delivery = ({
  ...
  const [activeTab, setActiveTab] = useState('Dispatch'); // 'Dispatch' or 'Zones'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <div style={{ display: 'flex', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden' }}>
            <button
              onClick={() => setActiveTab('Dispatch')}
              style={{
                padding: '8px 16px',
                border: 'none',
                backgroundColor: activeTab === 'Dispatch' ? 'var(--primary)' : 'transparent',
                color: activeTab === 'Dispatch' ? 'white' : 'var(--text-secondary)',
                cursor: 'pointer',
                fontWeight: '600',
                fontSize: '13px',
                transition: 'all 0.2s'
            ...
            <button
              ...
              style={{
                padding: '8px 16px',
                border: 'none',
                backgroundColor: activeTab === 'Zones' ? 'var(--primary)' : 'transparent',
                color: activeTab === 'Zones' ? 'white' : 'var(--text-secondary)',
                cursor: 'pointer',
                fontWeight: '600',
                fontSize: '13px',
                transition: 'all 0.2s'
          ...
          >
            <form onSubmit={handleAssignDriverSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <Select
                ...
                options={orders.filter(o => o.deliveryStatus !== 'Delivered' && o.deliveryStatus !== 'Cancelled').map(o => ({ value: o.id, label: `Order #${o.id} - ${o.customerName} (£${o.total})` }))}
              />
            </form>
          </Modal>
        </>
      ...
      {activeTab === 'Zones' && (
        <DeliveryZonesManager addToast={addToast} />
      )}
